...
def update_scheduler():
    ...
    logger.info(f"定时任务已配置：间隔 {interval_minutes} 分钟，刮削={scrape_enabled}，整理={organize_enabled}")
...
def run_scheduled_task():
    ...
    try:
        ...
        if scrape_enabled:
            ...
            add_op_log(now, "scrape", f"定时刮削完成：成功 {scrape_result['success']} 失败 {scrape_result['failed']} 跳过 {scrape_result['skipped']} · 耗时 {scrape_duration}")
        ...
        if organize_enabled:
            ...
            add_op_log(now, "organize", f"定时整理完成：移动 {organize_result['moved']} 跳过 {organize_result['skipped']} 失败 {organize_result['failed']} · 耗时 {organize_duration}")
...
def run_scrape_task():
    ...
    # 获取待处理的歌曲（缺少标签的）
    pending_tracks = get_pending_tracks()
...
def run_manual_task(task_type: str):
    ...
    try:
        ...
        if task_type == "scrape":
            ...
            scrape_duration = _format_duration(time.time() - scrape_start)
            add_op_log(now, "scrape", f"手动刮削完成：成功 {result['success']} 失败 {result['failed']} 跳过 {result['skipped']} · 耗时 {scrape_duration}")
            commit_track()
            ...
            organize_duration = _format_duration(time.time() - organize_start)
            add_op_log(now, "organize", f"手动整理完成：移动 {result['moved']} 跳过 {result['skipped']} 失败 {result['failed']} · 耗时 {organize_duration}")
            commit_track()
            ...
            scrape_result = run_scrape_task()
            scrape_duration = _format_duration(time.time() - scrape_start)
            add_op_log(now, "scrape", f"手动刮削完成：成功 {scrape_result['success']} 失败 {scrape_result['failed']} 跳过 {scrape_result['skipped']} · 耗时 {scrape_duration}")
            ...
            organize_result = run_organize_task()
            organize_duration = _format_duration(time.time() - organize_start)
            add_op_log(now, "organize", f"手动整理完成：移动 {organize_result['moved']} 跳过 {organize_result['skipped']} 失败 {organize_result['failed']} · 耗时 {organize_duration}")
