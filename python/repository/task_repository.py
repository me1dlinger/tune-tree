...
def get_task_status(task_type: str):
    """获取任务状态"""
    db = get_db()
    row = db.execute("SELECT * FROM task_status WHERE task_type=?", (task_type,)).fetchone()
    if row:
        return dict(row)
    # 返回默认状态
    return {
...
def update_task_status(task_type: str, status: str, is_manual: int = 0, error_message: str = None):
    """更新任务状态"""
    db = get_db()
    now = time.time()
    ...
    if current["id"]:
        db.execute(
            ...
            WHERE task_type=?
            """,
            (status, now, last_success_at, last_failure_at, error_message, 
             run_count, success_count, failure_count, is_manual, now, task_type),
        )
    else:
        db.execute(
            """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (task_type, status, now, last_success_at, last_failure_at, 
             error_message, run_count, success_count, failure_count, is_manual, now),
        )
